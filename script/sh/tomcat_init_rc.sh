...
# used on clr-dev, clr-test

## start/stop tomcat, as user "apache"  (formerly prog)
##
## tin.ho@novartis.com
##
...
# chkconfig --level 01246 tomcat off
#
RETVAL=0
prog="tomcat"
PROG="tomcat"
USARIO="apache-tomcat"		# expect local user UID 48, dup of apache, with shell
TOUCH=/bin/touch
SUBSYSFILE=/var/lock/subsys/tomcat


GRP=sys_nibr_jboss ## old version of su does not support -g option!!
#CATALINA_HOME=/usr/local/apache-tomcat-current/
#CATALINA_HOME=/app/apache-tomcat-current
CATALINA_HOME=/app/apache-tomcat-latest
JAVA_HOME="/usr/prog/java/jdk1.6.0_38/"
...
start()
{
  ...
  then
	 ...
	 su -m  $USARIO -c ${CATALINA_HOME}/bin/startup.sh
  else
	echo "Unable to start Tomcat,  ${CATALINA_HOME}/bin/startup.sh not found!"
  fi
}
...
case "$1" in
  start)
    $LOGGER -p local7.info "Starting Tomcat.  Args: $0 $1"
    start
    ;;
  stop)
    $LOGGER -p local7.info "Stoping Tomcat.  Args: $0 $1"
    stop
    ;;
  *)
