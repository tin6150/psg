...
## rclone ls dchiang50gdrv:/RCLONE_BK_via_SYNC
...
echo $RCLONE mkdir $REMOTE_NAME:$ROOT_FOLDER/$LOCAL_BACKUP ## > /dev/null 2>&1
echo $RCLONE --transfers=$TRANSFER_COUNT --checkers=$CHECKER_COUNT sync $LOCAL_BACKUP  ${REMOTE_NAME}:${ROOT_FOLDER}/${LOCAL_BACKUP} | tee -a $LOGFILE
		EXIT_CODE=$?
